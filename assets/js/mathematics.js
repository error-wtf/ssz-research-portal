(() => {
  ...
  function grid(){
    const m=+$("math-grid-m").value,a=+$("math-grid-angle").value*Math.PI/180,{c,w,h}=surface("math-grid-canvas"),scale=Math.min(w,h)*.68/Math.max(1,m-1),cx=w/2,cy=h/2,pts=[];
    for(let y=0;y<m;y++)for(let x=0;x<m;x++){const X=(x-(m-1)/2)*scale,Y=(y-(m-1)/2)*scale;pts.push([cx+X*Math.cos(a)-Y*Math.sin(a),cy+X*Math.sin(a)+Y*Math.cos(a)]);}
    c.strokeStyle="#64748b88";for(let y=0;y<m;y++)for(let x=0;x<m;x++){const i=y*m+x;if(x<m-1){c.beginPath();c.moveTo(...pts[i]);c.lineTo(...pts[i+1]);c.stroke();}if(y<m-1){c.beginPath();c.moveTo(...pts[i]);c.lineTo(...pts[i+m]);c.stroke();}}
    const reveal=Math.min(pts.length,1+Math.floor((tick%300)/300*pts.length));for(const [x,y] of pts.slice(0,reveal)){c.fillStyle="#bb8b2f";c.beginPath();c.arc(x,y,4,0,Math.PI*2);c.fill();}$("math-grid-m-out").textContent=m;$("math-grid-angle-out").textContent=Math.round(a*180/Math.PI)+"°";$("math-grid-points").textContent=m*m;$("math-grid-edges").textContent=2*m*(m-1);
  }
