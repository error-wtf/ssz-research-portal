(() => {
  ...
  async function falsification(){
    ...
    const render=()=>{const d=data.domains[select.value||0],stages=data.stage_order.map(key=>({key,status:d.stages[key]}));$("falsification-pipeline").innerHTML=stages.map(s=>`<div class="maturity-stage ${esc(s.status)}"><strong>${esc(s.key.replaceAll("_"," "))}</strong><span>${esc(s.status)}</span></div>`).join("");const ready=["forward_model","real_data","uncertainty_model","model_comparison"].every(k=>d.stages[k]==="complete"),tested=stages.filter(s=>s.status==="complete").length;$("falsification-verdict").className=`callout ${ready?"tested":"nuance"}`;$("falsification-verdict").innerHTML=`<strong>${ready?"Decision-chain implemented":"Active evidence chain"} · ${tested}/${stages.length} stages complete:</strong> ${esc(d.audit_note)} ${ready?"A preregistered quantitative comparison can now define the decision threshold.":"Completed stages remain positive evidence; partial or open stages identify the exact next validation step rather than erasing existing results."}`;};select.addEventListener("change",render);render();
  }
  ...
  function updateSandbox(){
    ...
    const checks=[
      ...
      ["Action, Lagrange and Hamilton formulation",true,"available in ssz-lagrange · 54/54 local rerun"],
      ["Energy-condition calculations",true,"WEC samples tested; segmented-energy linked above"],
      ["Geodesic equations and conservation",true,"implemented; finite-domain conservation tests pass"],
      ["Causal and global-extension research",true,"affine geodesics, Raychaudhuri focusing, nested-metric causal structure and Penrose-condition studies exist; completion status depends on the selected inner geometry"]
    ];
    ...
    $("sandbox-verdict").innerHTML=`<strong>${localPass?"Passes the displayed local polynomial checks":"Fails at least one displayed local polynomial check"}.</strong> ${regular?"A(0)=1 removes the leading areal-centre deficit term for this ansatz.":"A(0)≠1 retains the leading r⁻² and r⁻⁴ term in this ansatz."} C² matching is numerically audited at tolerance ${tol.toExponential(0)}. Read this local result together with the existing SSZ action, energy-condition, affine-geodesic, Raychaudhuri, nested-metric causal-structure and lensing work. The global conclusion is selected-model dependent and cannot be inferred from this polynomial alone.`;
  ...
  function bind(){
    const sandboxHead=document.querySelector("#interior-sandbox .section-head");
    if(sandboxHead){
      sandboxHead.querySelector(".badge").textContent="Interior candidate laboratory";
      sandboxHead.querySelector("p").innerHTML="Construct a quintic interior candidate for A(r) on 0≤r≤r<sub>m</sub>. Six conditions fix the polynomial and its C² exterior matching. This local audit complements the wider SSZ dynamics and energy research; it does not replace it.";
      const related=document.createElement("div");related.className="callout tested";related.innerHTML='<strong>Related public research exists:</strong> <a href="https://github.com/error-wtf/ssz-lagrange" target="_blank" rel="noopener">ssz-lagrange</a> supplies an action principle, Lagrange/Hamilton formulations, geodesic equations, energy-condition calculations and a locally reproduced 54/54 passing suite. <a href="https://github.com/error-wtf/segmented-energy" target="_blank" rel="noopener">segmented-energy</a> provides tested energy analyses. <a href="https://github.com/error-wtf/ssz-lensing" target="_blank" rel="noopener">ssz-lensing</a> provides synthetic, sweep, noise and real-data lensing validation. This sandbox adds local polynomial matching checks.';
      sandboxHead.insertAdjacentElement("afterend",related);
      const oldScope=document.querySelector("#interior-sandbox .callout.warning");if(oldScope)oldScope.remove();
    }
