(() => {
  ...
  function integrate(){
    ...
    $("geo-points").textContent=trajectory.length;$("geo-turns").textContent=turns;$("geo-error").textContent=maxResidual.toExponential(2);$("geo-time").textContent=t.toFixed(5);$("geo-status").textContent=trajectory.length>20?"integrated":"domain failure";drawGeodesic();
  }
  function drawGeodesic(){
    const {c,w,h,text,muted,line,gold}=canvas("geodesic-canvas"),cx=w*.42,cy=h/2,scale=Math.min(w*.38,h*.43)/Math.max(8,n("geo-r0"));
    [1,1.8,2.2].forEach((r,i)=>{c.strokeStyle=i?"#7c3aed55":"#b4231855";c.beginPath();c.arc(cx,cy,r*scale,0,Math.PI*2);c.stroke();});c.strokeStyle=gold;c.lineWidth=2;c.beginPath();trajectory.forEach((p,i)=>i?c.lineTo(cx+p.x*scale,cy+p.y*scale):c.moveTo(cx+p.x*scale,cy+p.y*scale));c.stroke();c.fillStyle=text;c.beginPath();c.arc(cx,cy,6,0,Math.PI*2);c.fill();
    const L=w*.67,R=w-20,T=38,B=h-50,E=n("geo-e"),ell=n("geo-l"),mass=$("geo-type").value==="timelike"?1:0,A=x=>window.SSZ.dilation(x)**2,V=x=>A(x)*(mass+ell**2/x**2),max=Math.max(E**2,2);c.strokeStyle=line;c.beginPath();c.moveTo(L,T);c.lineTo(L,B);c.lineTo(R,B);c.stroke();c.strokeStyle="#2563eb";c.beginPath();for(let i=0;i<=500;i++){const x=.1+20*i/500,y=B-V(x)/max*(B-T);i?c.lineTo(L+(R-L)*i/500,y):c.moveTo(L,y);}c.stroke();c.strokeStyle="#b42318";c.setLineDash([4,4]);c.beginPath();c.moveTo(L,B-E**2/max*(B-T));c.lineTo(R,B-E**2/max*(B-T));c.stroke();c.setLineDash([]);c.fillStyle=muted;c.textAlign="center";c.fillText("trajectory",cx,h-15);c.fillText("effective potential / E²",L+(R-L)/2,h-15);
  }
  ...
  function updateSandbox(){
    const s=sandboxState=sandbox(),eps=1e-5,R=s.R(eps),K=s.K(eps),regular=Math.abs(s.a0-1)<1e-6;
    ...
    $("sandbox-verdict").innerHTML=`<strong>${regular&&s.minA>0?"Local candidate passes sampled centre conditions":"Candidate rejected by local checks"}.</strong> ${regular?"A(0)=1 removes the leading areal-centre deficit in this ansatz.":"A(0)≠1 produces the leading curvature divergence."} Matching is C² by construction. Energy conditions, field equations, stability, global causality and geodesic completeness are not tested.`;
    const {c,w,h,line,gold}=canvas("interior-sandbox-canvas"),L=58,Rp=w-20,T=35,B=h-50,xp=x=>L+x/s.rm*(Rp-L),yp=y=>B-y/1.5*(B-T);c.strokeStyle=line;c.beginPath();c.moveTo(L,T);c.lineTo(L,B);c.lineTo(Rp,B);c.stroke();c.strokeStyle=gold;c.lineWidth=2.5;c.beginPath();for(let i=0;i<=600;i++){const x=s.rm*i/600,y=s.A(x);i?c.lineTo(xp(x),yp(y)):c.moveTo(xp(x),yp(y));}c.stroke();c.strokeStyle="#2563eb";c.setLineDash([5,4]);c.beginPath();for(let i=0;i<=200;i++){const x=s.rm*(.7+.3*i/200),y=window.SSZ.dilation(x)**2;i?c.lineTo(xp(x),yp(y)):c.moveTo(xp(x),yp(y));}c.stroke();c.setLineDash([]);
  }
  function bind(){
    ...
    ["geo-type","geo-r0","geo-e","geo-l","geo-step"].forEach(id=>$(id).addEventListener("input",()=>{if(id!=="geo-type")$(`${id}-out`).textContent=n(id).toFixed(id==="geo-step"?4:id==="geo-r0"?2:3);}));$("geo-run").addEventListener("click",integrate);$("geo-export").addEventListener("click",()=>download("ssz-geodesic-trajectory.json",{parameters:{type:$("geo-type").value,r0:n("geo-r0"),E:n("geo-e"),L:n("geo-l"),step:n("geo-step")},trajectory,limitation:"geodesic of the declared static metric only"}));
    ["sandbox-a0","sandbox-a2","sandbox-rm"].forEach(id=>$(id).addEventListener("input",updateSandbox));$("sandbox-export").addEventListener("click",()=>download("ssz-hypothetical-interior.json",{parameters:{A0:sandboxState.a0,Asecond0:sandboxState.a2,matching_radius:sandboxState.rm},coefficients:sandboxState.coef,limitations:["hypothetical polynomial ansatz","no field equations","no energy-condition or global-completeness proof"]}));
